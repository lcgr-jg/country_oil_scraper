"""
Processor for SSB monthly petroleum product sales (Norway).
"""
...
from reference.norway import CANONICAL_COLUMNS, SSB_AGENCY_SOURCE
from scrapers.norway_ssb import NorwaySsbScraper

logger = logging.getLogger(__name__)

_PRODUCT_MAP_SOURCE = SSB_AGENCY_SOURCE
...
PARQUET_FILENAME = "norway_ssb_sales.parquet"
SQLITE_FILENAME = "norway_ssb_sales.db"
SQLITE_TABLE = "ssb_sales"


def build_from_historical(data_dir: Path) -> pd.DataFrame:
    """Bootstrap by stitching all StatBank eras via the scraper."""
    data_dir = Path(data_dir)
    scraper = NorwaySsbScraper(data_dir=data_dir)
    logger.info("Building Norway SSB DB from StatBank API (all eras)")
    df = scraper.bootstrap_historical()
...
def _derive_canonical_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if unknown:
        logger.warning(
            "  %d Norway product_native label(s) missing from product_map.csv: %s",
            len(unknown),
            sorted(unknown),
        )
