"""
Processor for BAFA Amtliche Mineralöldaten (Germany).

Stores demand (TOTDEMO), bio blends (BIOBLEND), and closing stocks (CLOSTLV)
in one parquet. Warehouse consolidate loads TOTDEMO only for the demand view.
"""
...
from reference.germany import BAFA_AGENCY_SOURCE, CANONICAL_COLUMNS
...
_PRODUCT_MAP_SOURCE = BAFA_AGENCY_SOURCE
...
PARQUET_FILENAME = "germany_bafa_demand.parquet"
SQLITE_FILENAME = "germany_bafa.db"
SQLITE_TABLE = "bafa"


def build_from_historical(data_dir: Path) -> pd.DataFrame:
    """Bootstrap from all local BAFA month files under data/raw/germany/."""
    data_dir = Path(data_dir)
    scraper = GermanyBafaScraper(data_dir=data_dir)
    logger.info("Building Germany BAFA DB from local month files in %s", scraper.raw_dir)
    df = scraper.parse_all_local()
...
def _derive_canonical_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if unknown:
        logger.warning(
            "  %d Germany product_native label(s) missing from product_map.csv: %s",
            len(unknown),
            sorted(unknown),
        )
