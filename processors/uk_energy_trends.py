"""
Processor for DESNZ Energy Trends UK consumption and product stocks.
"""
...
from reference.uk import CANONICAL_COLUMNS, DESNZ_AGENCY_SOURCE
from scrapers.uk_desnz import UkDesnzScraper

logger = logging.getLogger(__name__)

_PRODUCT_MAP_SOURCE = DESNZ_AGENCY_SOURCE
...
PARQUET_FILENAME = "uk_energy_trends.parquet"
SQLITE_FILENAME = "uk_energy_trends.db"
SQLITE_TABLE = "uk_energy_trends"


def build_from_historical(raw_path: Path) -> pd.DataFrame:
    """Bootstrap from a single consolidated Energy Trends ODS workbook."""
    raw_path = Path(raw_path)
    data_dir = raw_path.parents[2]
    scraper = UkDesnzScraper(data_dir=data_dir)
    logger.info("Building UK Energy Trends DB from %s", raw_path)
    df = scraper.parse("energy_trends", raw_path)
    df = _sort_and_clean(df)
    logger.info(
        "Built %s rows, %s -> %s, %s products, metrics=%s",
        f"{len(df):,}",
        df["date"].min(),
        df["date"].max(),
        df["product_native"].nunique(),
        sorted(df["metric_type"].dropna().unique()),
    )
    return df
...
def _derive_canonical_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if unknown:
        logger.warning(
            "  %d UK product_native label(s) missing from product_map.csv: %s",
            len(unknown),
            sorted(unknown),
        )
