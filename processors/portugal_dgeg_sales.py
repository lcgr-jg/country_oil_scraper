"""
Processor for DGEG monthly oil product sales (Portugal).
"""
...
from reference.portugal import CANONICAL_COLUMNS, DGEG_AGENCY_SOURCE
from scrapers.portugal_dgeg import PortugalDGEGScraper

logger = logging.getLogger(__name__)

_PRODUCT_MAP_SOURCE = DGEG_AGENCY_SOURCE
MIN_BOOTSTRAP_YEAR = 2006
...
PARQUET_FILENAME = "portugal_dgeg_sales.parquet"
SQLITE_FILENAME = "portugal_dgeg_sales.db"
SQLITE_TABLE = "dgeg_sales"


def build_from_historical(raw_dir: Path) -> pd.DataFrame:
    """Bootstrap from all workbooks under ``data/raw/portugal/``."""
    raw_dir = Path(raw_dir)
    data_dir = raw_dir.parents[1]
    scraper = PortugalDGEGScraper(data_dir=data_dir)
    paths = scraper.local_workbooks()
    if not paths:
        raise FileNotFoundError(f"No DGEG workbooks under {raw_dir}")
    logger.info("Building Portugal DGEG DB from %d workbook(s)", len(paths))
    df = scraper.parse_all(paths)
    df = df[df["date"].dt.year >= MIN_BOOTSTRAP_YEAR]
...
def _derive_canonical_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if unknown:
        logger.warning(
            "  %d Portugal product_native label(s) missing from product_map.csv: %s",
            len(unknown),
            sorted(unknown),
        )
