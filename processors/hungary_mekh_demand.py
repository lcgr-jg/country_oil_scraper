"""
Processor for MEKH monthly oil balance (Hungary) — demand + closing stocks.
"""
...
from reference.hungary import CANONICAL_COLUMNS, MEKH_AGENCY_SOURCE
from scrapers.hungary_mekh import HungaryMekhScraper

logger = logging.getLogger(__name__)

_PRODUCT_MAP_SOURCE = MEKH_AGENCY_SOURCE
...
PARQUET_FILENAME = "hungary_mekh_demand.parquet"
SQLITE_FILENAME = "hungary_mekh.db"
SQLITE_TABLE = "mekh"


def build_from_historical(data_dir: Path) -> pd.DataFrame:
    """Bootstrap from cached OData snapshots."""
    data_dir = Path(data_dir)
    scraper = HungaryMekhScraper(data_dir=data_dir)
    if not scraper.demand_snapshot_path.exists():
        raise FileNotFoundError(
            f"No demand snapshot at {scraper.demand_snapshot_path}. "
            "Run update without --no-download."
        )
    if not scraper.stocks_snapshot_path.exists():
        raise FileNotFoundError(
            f"No stocks snapshot at {scraper.stocks_snapshot_path}. "
            "Run update without --no-download."
        )
    logger.info("Building Hungary MEKH DB from OData snapshots")
    df = scraper.parse_all()
...
def _derive_canonical_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if unknown:
        logger.warning(
            "  %d Hungary product_native label(s) missing from product_map.csv: %s",
            len(unknown),
            sorted(unknown),
        )
